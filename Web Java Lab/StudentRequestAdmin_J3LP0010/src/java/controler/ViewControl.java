...
DAO dao = new DAO();
        ...
        if (request.getParameter("today") == null && request.getParameter("requestTo") == null) {
            listRequest = dao.getViewRequestDepartmentByDid(1);
        } else {
            int to = Integer.parseInt(requestTo);
            int isToday = Integer.parseInt(today);

            if (isToday == 1) {
                listRequest = dao.getViewRequestDepartmentToday(to);
            } else {
                listRequest = dao.getViewRequestDepartmentTime(to);
            }
        }
